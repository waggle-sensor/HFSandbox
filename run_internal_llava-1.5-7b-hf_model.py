...
def main():
    ...
    for FILE in files:
        if FILE not in CHECKPOINT:
            ...
            image_file=FILE
            pattern='###Assistant:'
            response, time=run_llava(model, processor, user_prompt, image_file)
            internal_rep, time=internal_run_llava(model, processor, user_prompt, image_file)
            rgb_response, time=rgb_run_llava(model, processor, user_question, user_prompt, image_file)
            rgb_internal_rep, rgb_time=internal_rgb_run_llava(model, processor, user_prompt, image_file)
            ir_response, time=ir_run_llava(model, processor, user_prompt, image_file)
            ir_internal_rep, ir_time=internal_ir_run_llava(model, processor, user_prompt, image_file)
